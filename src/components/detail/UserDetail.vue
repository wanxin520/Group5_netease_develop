...
import { getUserAccount, getUserSubcount, getUserDetail } from "@/api/userIndex";
...
const { data: userDetail, loading: userDetailLoading } = useRequest(() => getUserDetail({ "timestamp": Date.now(), "uid": props.account.id }))
watch(userDetail, () => {
...
<template>
    <div v-if="!userDetailLoading" class="h-[100vh]">
        <div class="h-[50%] flex flex-col justify-around items-center"
            :style="{ backgroundImage: `url(${userDetail.profile.backgroundUrl})` }">
            <div class="w-[100%]">
                <slot></slot>
            </div>
            <div class="w-[100%] flex flex-col justify-around items-center ">
                <div class="w-[100%] flex justify-center">
                    <img class="w-[5rem] h-[5rem] rounded-[50%]" :src="userDetail.profile.avatarUrl" alt="">
                </div>
            </div>
