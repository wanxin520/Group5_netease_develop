...
const { data, loading } = useRequest(() => getRecommendMV());
watch(data, () => {
...
<template>
  <div v-if="!loading" class="w-[100vw] flex justify-center items-center mt-2">
    <van-swipe :width="350" :height="80" :stop-propagation="false" :loop="false" :show-indicators="false" vertical>
      <van-swipe-item class="p-1" @click="getSourceId(item.id)" v-for="(item, index) in data.result" :key="item.id">
        <div class="w-[100%] flex justify-start items-center rounded-[2rem]">
          <div class="w-[6rem] h-[4rem] flex justify-center items-center"
            :style="`background-image: url(${item.picUrl}); background-size: cover; border-radius: 10px;`">
            <Icon icon="iconoir:play-solid" width="1.3rem" class="text-[#f1f1f1de]" />
          </div>
          <div class="ml-3 w-[60%]">
            <div class="w-[100%] text-[#323232] text-[12px] font-bold overflow-hidden text-ellipsis whitespace-nowrap ">
              {{ item.name }}
            </div>
            ...
            <div class="flex justify-start items-center mt-1 text-[#636363] text-[8px]">
              <span>
                {{ new Date(item.duration).getMinutes() < 10 ? "0" + new Date(item.duration).getMinutes() : new
                  Date(item.duration).getMinutes() }}:{{ new Date(item.duration).getSeconds() < 10 ? "0" + new
                    Date(item.duration).getSeconds() : new Date(item.duration).getSeconds() }} </span>
                  <span>，</span>
