...
import { getKoreaMV } from "@/api/videoIndex";
import { useRequest } from "vue-request";

const { data, loading } = useRequest(() => getKoreaMV());
watch(data, () => {
...
<template>
  <div v-if="!loading" class="w-[100vw] flex justify-center items-center mt-2">
    ...
    >
      <van-swipe-item
        class="p-1"
        @click="getSourceId(item.id)"
        v-for="(item, index) in data.data"
        :key="item.id"
      >
        <div class="w-[100%] flex justify-start items-center rounded-[2rem]">
          <div
            class="w-[6rem] h-[4rem] flex justify-center items-center"
          ...
          <div class="ml-3 w-[60%]">
            ...
            <div
              class="flex justify-start text-[10px] items-center font-mono mt-1"
            >
              {{ item.artists[0].name }}
            </div>
            <div
              class="flex justify-start items-center mt-1 text-[#636363] text-[8px]"
            >
              <span>
                {{
