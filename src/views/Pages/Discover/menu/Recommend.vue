            ...
            <van-swipe :stop-propagation="false" :loop="false" :show-indicators="false">
                <van-swipe-item v-for="(item) in PageData" :key="item">
                    <div class="flex flex-col items-center">
                        <span>
                            <h1>{{ item.copywriter }}</h1>
                        </span>
                        <div class="flex flex-col items-center">
                            <div class=" flex justify-between w-[100vw]">
                                <img class="rounded-[20px]  w-[30vw] h-[30vw]" :src="item.picUrl" alt=""
                                    @click="getuserId(item.userId)" />
                                <img class="rounded-[20px]  w-[30vw] h-[30vw]" :src="item.creator.avatarUrl" alt=""
                                    @click="getavatarImgId(item.creator.avatarImgId)" />
                                <img class="rounded-[20px]  w-[30vw] h-[30vw]" :src="item.creator.backgroundUrl" alt=""
                                    @click="getBackgroundId(item.creator.backgroundImgId)" />
                            </div>
