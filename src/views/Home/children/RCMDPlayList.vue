...
<template>
  ...
  <div class="w-[100vw] flex justify-center items-center">
    <van-swipe :width="150" :stop-propagation="false" :loop="false" :show-indicators="false">
      <van-swipe-item class="p-2" v-for="(item, index) in props.data.children" :key="item.id">
        <div @click="isChecked(item.id)" class="flex flex-col items-center">
          <div>
            <div
              ...
              <div class="" v-if="props.data.resourceExtInfo[index].playCount > 100000000">
                {{
                  `${(props.data.resourceExtInfo[index].playCount / 100000000).toFixed(1)}亿`
                }}
              </div>
              <div v-else-if="props.data.resourceExtInfo[index].playCount > 10000">
                {{
                  `${(props.data.resourceExtInfo[index].playCount / 10000).toFixed(2)}万`
                }}
              </div>
              <div v-else>
                {{
