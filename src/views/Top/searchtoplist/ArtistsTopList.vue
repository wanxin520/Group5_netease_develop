...
import { getArtistTopList } from "@/api/searchIndex";


const { data, loading } = useRequest(() => getArtistTopList())

watch(data, () => {
    // console.log(data.value.list.artists);
})
...
<template>
    <div>
        <div v-if="!loading" class="w-[100%] flex flex-col justify-center items-center">
            ...
            <div class="w-[100%]">
                <van-cell v-for="(item, index) in data.list.artists.slice(0,12)" :border="false" :key="item">
                    <!-- 使用 title 插槽来自定义标题 -->
                    <template #title>
                    ...
                    <template v-if="item.img1v1Url" #right-icon>
                        <div class="flex justify-center items-center">
                            <img class="w-[2rem] h-[2rem] rounded-[5px]" :src="item.img1v1Url" alt="">
                        </div>
                    </template>
                </van-cell>
            </div>
