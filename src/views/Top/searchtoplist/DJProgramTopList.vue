...
import { getDJProgramTopList } from "@/api/searchIndex";


const { data, loading } = useRequest(() => getDJProgramTopList())
...
<template>
    <div>
        <div v-if="!loading" class="w-[100%] flex flex-col justify-center items-center">
            ...
            <div class="w-[100%]">
                <van-cell v-for="(item, index) in data.toplist.slice(0, 12)" :border="false" :key="item">
                    <!-- 使用 title 插槽来自定义标题 -->
                    <template #title>
